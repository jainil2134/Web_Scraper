...
class SecuritySpider(scrapy.Spider):
    ...
    def __init__(self, url=None, filename=None, audit_metadata=None, *args, **kwargs):
        ...
        # FIX 1: Initialize final_data immediately so it's never empty
...
def run_audit():
    ...
    process = CrawlerProcess(settings={
        "LOG_LEVEL": "ERROR", 
        # FIX 3: Realistic User-Agent to avoid immediate blocks
        "USER_AGENT": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "ROBOTSTXT_OBEY": False # Meesho might Disallow / in robots.txt
    })

    process.crawl(SecuritySpider, url=user_url, filename=target_filename, audit_metadata=audit_metadata)
    process.start() 

    # Final check to see if file exists before reading
