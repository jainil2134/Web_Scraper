...
class SecuritySpider(scrapy.Spider):
    ...
    def __init__(self, url=None, filename=None, audit_metadata=None, *args, **kwargs):
        ...
        self.final_data["findings"] = {
            ...
            "security_header_audit": {} # Storage for HSTS/CSP check
        }
    ...
    def parse(self, response):
        ...
        # --- NEW: Security Header Audit ---
        hsts = resp_headers.get('Strict-Transport-Security', 'MISSING')
        csp = resp_headers.get('Content-Security-Policy', 'MISSING')
        ...
        detected = [h for h in proxy_headers if h in resp_headers]
        if detected or "Cloudflare" in resp_headers.get('Server', ''):
...
def run_audit():
    ...
    process = CrawlerProcess(settings={"LOG_LEVEL": "ERROR", "USER_AGENT": "Mozilla/5.0"})
    ...
    if os.path.exists(target_filename):
        ...
        # --- ROBUST SUMMARY TABLE LOGIC ---
        # Get the audit dictionary, or an empty dict if it's missing entirely
        header_audit = data.get("findings", {}).get("security_header_audit", {})

        # Use .get() to avoid KeyError if the specific security headers aren't in the dict
